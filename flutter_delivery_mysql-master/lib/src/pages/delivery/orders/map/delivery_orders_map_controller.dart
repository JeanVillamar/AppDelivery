...
Future<void> setPolylines(LatLng from, LatLng to) async {
    points.clear(); // si no limpias, se te acumulan rutas viejas

    final pointFrom = PointLatLng(from.latitude, from.longitude);
    final pointTo = PointLatLng(to.latitude, to.longitude);
    ...
    if (result.points.isEmpty) {
      // si quieres, imprime errorMessage para ver qué pasó (API key, billing, etc.)
    ...
    polylines.clear(); // opcional: si solo quieres 1 ruta
    polylines.add(
      Polyline(
  ...
  Future<void> callNumber() async {
    final raw = order.client?.phone ?? '';
    final number =
        raw.replaceAll(RegExp(r'[^0-9+]'), ''); // limpia espacios/guiones

    if (number.isEmpty) {
      Fluttertoast.showToast(msg: 'No hay número para llamar');
      return;
    }
    ...
    if (!await canLaunchUrl(uri)) {
      Fluttertoast.showToast(msg: 'No se pudo abrir el marcador');
      return;
    }
